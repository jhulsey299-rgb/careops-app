        ...
        lessons: [
          {
            kind: "concept",
            id: "f1",
            title: "What Counts as Hospital Data?",
            objective: "Understand the main kinds of hospital data analysts use and why encounter-level data often anchors operations reporting.",
            sql_focus: ["SELECT", "FROM", "LIMIT"],
            relevantTables: ["encounters"],
            joinHint: "No join is needed for this lesson. Start with one operational table so you can study what one row represents.",
            summary: "Hospital analysts work across clinical, operational, and financial data, but safe analysis starts by choosing the right business grain before calculating anything.",
            bullets: [
              "Operational questions often begin with encounters because one row usually represents one visit or stay.",
              "Financial questions may begin with charges or claims because those tables represent transactions rather than visits.",
              "The first analyst mistake is often not SQL syntax. It is starting from the wrong table and answering the wrong business question."
            ],
            example: "SELECT encounter_id, patient_id, department, encounter_type, status FROM encounters LIMIT 5;",
            executiveTakeaway: { show: false }
          },
          {
            kind: "challenge",
            id: "f2",
            title: "Preview Encounter-Level Records",
            objective: "Pull a clean encounter-level sample so you can confirm what one row represents.",
            sql_focus: ["SELECT", "FROM", "LIMIT"],
            relevantTables: ["encounters"],
            joinHint: "Use only the encounters table. This lesson is about validating encounter grain before building metrics.",
            challengeCriteria: `An operations manager wants to review a small encounter-level sample before asking for throughput metrics.

Return encounter_id, patient_id, department, encounter_type, and status from the encounters table.
Limit the result to 10 rows.

This helps confirm that one row in the table represents one encounter rather than one patient or one financial transaction.`,
            starterQuery: "SELECT encounter_id, patient_id, department, encounter_type, status FROM encounters LIMIT 10;",
            solutionQuery: "SELECT encounter_id, patient_id, department, encounter_type, status FROM encounters LIMIT 10;",
            hint: "Select the five requested columns from encounters and limit the output to 10 rows.",
            smartHint: "Use SELECT encounter_id, patient_id, department, encounter_type, status FROM encounters LIMIT 10.",
            thirdHint: "SELECT encounter_id, patient_id, department, encounter_type, status FROM encounters LIMIT 10;",
            explanation: `This query previews encounter-level records without adding unnecessary logic.

That matters because analysts usually validate grain first before calculating LOS, readmissions, throughput, or volume. When one row equals one encounter, visit-based metrics make sense.`,
            executiveTakeaway: { show: false }
          },
          {
            kind: "concept",
            id: "f3",
            title: "Patients Versus Encounters: Why Grain Matters",
            objective: "Understand the difference between patient-level and encounter-level analysis.",
            sql_focus: ["SELECT", "FROM", "LIMIT"],
            relevantTables: ["patients", "encounters"],
            joinHint: "No join is needed yet. Compare the role of patients and encounters before combining them later.",
            summary: "A patient can have many encounters, so patient counts and encounter counts answer different business questions.",
            bullets: [
              "Use patients when the question is about people, such as payer mix by person or high-risk populations.",
              "Use encounters when the question is about visits, stays, workload, ED volume, admissions, or throughput.",
              "If you confuse patient grain and encounter grain, you can overstate the population by counting repeat visits as new people."
            ],
            example: "SELECT patient_id, first_name, last_name, insurance_type, risk_score FROM patients LIMIT 5;",
            executiveTakeaway: { show: false }
          },
          {
            kind: "challenge",
            id: "f4",
            title: "Pull Patient Master Records",
            objective: "Pull a patient-level sample so you can distinguish patient data from encounter data.",
            sql_focus: ["SELECT", "FROM", "LIMIT"],
            relevantTables: ["patients"],
            joinHint: "Use only the patients table. This lesson is about confirming one row per patient.",
            challengeCriteria: `A population health analyst wants a quick patient-level sample to understand what demographic and payer fields are available.

Return patient_id, first_name, last_name, insurance_type, and risk_score from the patients table.
Limit the result to 10 rows.

This helps confirm that one row in the patients table represents one patient rather than one visit.`,
            starterQuery: "SELECT patient_id, first_name, last_name, insurance_type, risk_score FROM patients LIMIT 10;",
            solutionQuery: "SELECT patient_id, first_name, last_name, insurance_type, risk_score FROM patients LIMIT 10;",
            hint: "Select the five requested columns from patients and limit the output to 10 rows.",
            smartHint: "Use SELECT patient_id, first_name, last_name, insurance_type, risk_score FROM patients LIMIT 10.",
            thirdHint: "SELECT patient_id, first_name, last_name, insurance_type, risk_score FROM patients LIMIT 10;",
            explanation: `This query reinforces patient grain.

That matters because patient-level analysis supports segmentation, attribution, and risk review. If you answer a patient question with encounter data, you may count the same person multiple times.`,
            executiveTakeaway: { show: false }
          },
          {
            kind: "scenario",
            ...
            objective: "Explain which table should be used first when leadership asks about ED visit volume versus unique patients.",
            relevantTables: ["patients", "encounters"],
            joinHint: "Think first about what each row represents. Visit volume is not the same as unique people served.",
            summary: "A director asks two related questions in the same meeting: how many ED visits occurred, and how many unique patients those visits represented.",
            prompt: "Explain which table you would start with for ED visit volume and which approach you would use for unique patients. In your response, mention encounter grain, patient grain, and why counting visits is different from counting people.",
            expectedKeywords: ["encounter", "patient", "grain", "visit", "distinct"],
            minLength: 100,
            minimumKeywordMatches: 3,
            feedbackGuide: "A strong answer explains that encounters answer visit-volume questions, while unique-patient questions require patient-level thinking or a DISTINCT patient count from encounter data.",
            executiveTakeaway: { show: false }
          }
        ]
      }
